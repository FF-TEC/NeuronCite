...
\fancyhead[R]{\thepage}
\fancyfoot[C]{\small Version 0.1.1 -- March 2026}
...
\begin{enumerate}[noitemsep]
    ...
    \item LaTeX citation verification: parses \texttt{\textbackslash cite} commands
          ...
          manages a batch-based claim/submit workflow with seven verdict types
    \item PDF annotation: highlights quoted text passages and adds comment
          annotations via a 5-stage text matching pipeline (exact, normalized,
          fuzzy, fallback extraction, OCR)
...
\begin{longtable}{>{\ttfamily\small}p{3.8cm} p{1.4cm} p{9.3cm}}
    ...
    neuroncite-annotate & Library & PDF annotation pipeline: highlights quoted
                                ...
                                through a 5-stage pipeline (exact, normalized,
                                fuzzy, fallback extraction, OCR), and creates
                                pdfium highlight and text annotations. \\
    neuroncite-citation & Library & LaTeX citation verification pipeline.
                                ...
                                seven verdict types and LaTeX correction
                                suggestions. \\
    ...
    neuroncite        & Binary  & The main entry point. Parses CLI arguments via
                                ...
                                index, search, doctor, sessions, export,
                                annotate, version, models, mcp). In web mode,
...
\begin{longtable}{>{\ttfamily\small\raggedright\arraybackslash}p{5.2cm}
    ...
    src/locate.rs   & private & 5-stage text location pipeline. Stage~1: exact
                      ...
                      Stage~3.5: fallback extraction --- re-extracts page text
                      using the neuroncite-pdf multi-backend pipeline and
                      re-attempts stages~1--3 on the fresh text.
...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{Text Location:} A 5-stage pipeline locates each quote in the PDF text
          (described below).
...
\subsection{5-Stage Text Location Pipeline}
...
\begin{designbox}[Stage 3.5: Fallback Extraction]
When pdfium text extraction yields no match in stages~1--3, re-extracts page text
using the neuroncite-pdf multi-backend pipeline (poppler, mupdf, pdfium fallback
chain). Stages~1--3 are re-attempted on the freshly extracted text. This captures
content that pdfium's primary extraction missed due to encoding differences between
...
Seven verdict types capture the sub-agent's assessment:
...
\begin{description}[style=nextline, labelwidth=3cm, leftmargin=3.3cm, noitemsep]
    ...
    \item[\texttt{peripheral\_match}] The claim text was found in the cited source,
         but exclusively in non-substantive sections (table of contents, bibliography,
         index, foreword, appendix, glossary).
         Color: \texttt{\#FFC107} (amber).
\end{description}
...
\begin{longtable}{>{\ttfamily\raggedright\arraybackslash}p{3.2cm}
    ...
    match\_method    & TEXT                & The location method that found the quote:
                       ...
                       ``fallback\_extract'', or ``ocr''. NULL while
...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.52\textwidth}X}
    ...
    \midrule
    \texttt{cargo build -{}-release} & Builds with default features (backend-ort, web, gui, mcp, pdfium, ocr). \\
