...
The application ships as a single executable file that contains a browser-based
SolidJS frontend, an HTTP REST API server, and a command-line interface with subcommands.
Double-clicking the executable starts the web server and opens the default browser.
...
\begin{tabularx}{\textwidth}{lX}
    ...
    OCR & Tesseract CLI subprocess (auto-downloaded on first use) \\
...
The project is organized as a Cargo workspace with fourteen library crates, one binary
crate, and one standalone test-fixture generator (\texttt{neuroncite-testgen}).
Each library crate encapsulates a single architectural concern.
...
\begin{longtable}{>{\ttfamily\small}p{3.8cm} p{1.4cm} p{9.3cm}}
    ...
    neuroncite        & Binary  & The main entry point. Parses CLI arguments via
                                ...
                                opens the default browser. In headless mode, runs
...
\begin{tikzpicture}[x=1cm, y=1cm,
    ...
    % Most library crates depend on neuroncite-core directly (11 of 14).
...
\begin{longtable}{>{\ttfamily\small\raggedright\arraybackslash}p{5.2cm}
    ...
    src/ocr.rs      & public    & Renders pages to raster images at adaptive DPI via
                      ...
                      Tesseract CLI binary as a subprocess. Tesseract is located
                      via PATH or auto-downloaded on first use. \\
...
\begin{enumerate}[noitemsep]
    ...
    \item The rendered PNG image is written to a temporary file and passed to the
          ...
          Tesseract is located on the system PATH or in the auto-download cache.
          If not found, it is downloaded automatically on first use (UB-Mannheim
          installer on Windows, AppImage on Linux).
...
\begin{tabularx}{\textwidth}{lX}
    ...
    \texttt{web} (default) & Compiles the \texttt{neuroncite-web} crate and pulls in
        ...
        embedded SolidJS frontend and opens the default browser. The
    ...
    \texttt{ocr} (default) & Enables Tesseract-based OCR fallback for image-heavy pages in
        \texttt{neuroncite-pdf}. Tesseract is invoked as a CLI subprocess and
        auto-downloaded on first use if not found on PATH. Implies the
        \texttt{pdfium} feature for page rendering to raster images. Defined on
        \texttt{neuroncite-pdf}, forwarded by the binary crate. \\
...
\begin{tabularx}{\textwidth}{>{\ttfamily\raggedright\arraybackslash}p{3cm}X}
    ...
    web             & Starts the Axum HTTP server with the embedded SolidJS
                      frontend and opens the default browser. This is the default
                      mode when no subcommand is given (double-click behavior on
                      Windows). Accepts \texttt{-{}-port}, \texttt{-{}-bind},
